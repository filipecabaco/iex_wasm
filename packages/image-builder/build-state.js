...
var OUTPUT_FILE = path.join(V86_ROOT, '/system/state.bin')
...
var emulator = new V86({
  ...
  filesystem: {
    basefs: {
      url: path.join(V86_ROOT, '/system/filesystem.json'),
    },
    baseurl: path.join(V86_ROOT, '/system/filesystem/'),
  },
  screen_dummy: true,
})
...
emulator.add_listener('serial0-output-char', function (c) {
  ...
  if (!booted && serial_text.endsWith('iex(1)> ')) {
    emulator.serial0_send('\x03')
    emulator.serial0_send('a')
